...
const SETTINGS = ['2 Column Layout', '3 Column Layout'];
...
export class AppComponent implements OnInit {
  ...
  constructor() {
    ...
    const savedWidths = localStorage.getItem('jsonapi-explorer.columnWidths');
    if (savedWidths) {
      try {
    ...
    effect(() => {
      ...
      localStorage.setItem('jsonapi-explorer.columnWidths', JSON.stringify(widths));
    });
  }
  ...
  getSettingsIconCheckStyle(setting: string): string {
    const isActive = setting.includes(this.appService.getColumnLayout());
  ...
  initializeColorMode(): void {
    const storedColorMode = localStorage.getItem('jsonapi-explorer.colorMode') as ColorMode | null;
  ...
  setColorMode(mode: ColorMode): void {
    ...
    localStorage.setItem('jsonapi-explorer.colorMode', mode);
    this.applyColorMode();
  }
